...
library AerodromeLibrary {
    ...
    function liquidatePosition(
        ...
        uint256 amount1Min,
        address recipient
    ) internal returns (address token0, address token1, int24 tickSpacing, uint256 amount0, uint256 amount1) {
        // get position info
        uint128 liquidity;
        (,, token0, token1, tickSpacing,,, liquidity,,,,) = positionManager.positions(positionId);
    ...
    function swap(
        ...
        inputs[0] =
            abi.encode(recipient, amountIn, amountOutMin, abi.encodePacked(tokenIn, tickSpacing, tokenOut), true);

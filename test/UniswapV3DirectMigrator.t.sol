...
contract UniswapV3DirectMigratorTest is TestContext, UniswapV3Helpers {
    ...
    function setUp() public {
        ...
        vm.prank(owner);
        settler = new UniswapV3DirectSettler(
            owner,
    ...
    */
    ...
    function test_onERC721Received_Token0WETHBaseToken_InRange() public {
        ...
        for (uint256 i = 0; i < entries.length; i++) {
            if (entries[i].topics.length > 0 && entries[i].topics[0] == IMigrator.MigrationStarted.selector) {
                ...
                (address loggedSettler, MigrationMode loggedMode, address loggedSender, address loggedToken, uint256 loggedAmount) = abi.decode(entries[i].data, (address, MigrationMode, address, address, uint256));
    ...
    function test_onERC721Received_Token0WETHBaseToken_BelowCurrentTick() public {
        ...
        IMigrator.MigrationParams memory migrationParams =
            generateDirectMigrationParams(token0, address(settler), 1);
    ...
    function test_onERC721Received_Token0WETHBaseToken_Token1USDCBaseToken_InRange() public {
        ...
        IMigrator.MigrationParams memory migrationParams = generateDirectMigrationParams(
            token0, token1, token0, token1, amount0 - 1, amount1 - 1, address(settler)
        );
    ...
    // Helper functions
    function findTransferEvent(Vm.Log[] memory entries, address from, address to) internal pure returns (Vm.Log memory) {
        for (uint256 i = 0; i < entries.length; i++) {
            if (entries[i].topics[0] == IERC20.Transfer.selector &&
    ...
    function findTransferEvents(Vm.Log[] memory entries, address from, address to) internal pure returns (Vm.Log[] memory) {

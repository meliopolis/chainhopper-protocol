...
contract UniswapV4SettlerTest is TestContext {
    ...
    function test_mintPosition_dualRoute_refundWeth() public {
        deal(address(settler), 1_000);
        deal(weth, address(settler), 100);

        bytes memory data = abi.encode(
            IUniswapV4Settler.MintParams(address(0), weth, 100, 1, address(0), 1 << 96, -600, 600, 5_000_000, 0, 0)
        );

        settler.mintPosition(address(0), weth, 1_000, 100, user, data);
    }

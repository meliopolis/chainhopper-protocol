...
import {IAerodromeNonfungiblePositionManager} from
...
contract AerodromeMigratorTest is TestContext, AerodromeHelpers {
    ...
    function test_fuzz_onERC721Received(address from, uint256 tokenId, bytes memory data) public {
        vm.prank(address(aerodromePositionManager));
    ...
    function test_liquidate() public {
        (uint256 positionId,,) =
            mintAerodromePosition(address(aerodromePositionManager), user, weth, usdc, -300000, 200000, tickSpacing, 0);
        vm.prank(user);
        IERC721(address(aerodromePositionManager)).approve(address(migrator), positionId);
    ...
    function test_liquidate_returnsCorrectPoolInfo() public {
        ...
        // Decode pool info
        (address poolToken0, address poolToken1, int24 poolTickSpacing) =
            abi.decode(poolInfo, (address, address, int24));

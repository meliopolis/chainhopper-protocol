...
contract SingleTokenV3V3SettlerTest is Test {
    LPMigrationSingleTokenHandler public handler;
    ...
    function setUp() public {
        ...
        handler = new LPMigrationSingleTokenHandler(nftPositionManager, baseToken, swapRouter, spokePool);
    }
    ...
    function generateSettlementParams(
        ...
        uint256 amount0Min,
        uint256 amount1Min
    ) public view returns (ILPMigrator.SettlementParams memory) {
        return ILPMigrator.SettlementParams({
            token0: token0,
            token1: token1,
            feeTier: feeTier,
            tickLower: tickLower,
    ...
    function test_handleV3AcrossMessage_msgSenderIsNotSpokePool() public {
        vm.prank(user);
        vm.expectRevert(ILPMigrationHandler.OnlySpokePoolCanCall.selector);
        handler.handleV3AcrossMessage(baseToken, 100, address(0), new bytes(0));
    }

    function test_handleV3AcrossMessage_tokenSentIsNotBaseToken() public {
        vm.prank(spokePool);
        vm.expectRevert(ILPMigrationHandler.OnlyBaseTokenCanBeReceived.selector);
        handler.handleV3AcrossMessage(usdc, 100, address(0), new bytes(0));
    }
    ...
    function test_handleV3AcrossMessage_trySwapAndCreatePositionFailsAndTriggersCatch() public {
        ...
        deal(baseToken, address(handler), 1 ether);
        // invalid settlement params for above tick
        ILPMigrator.SettlementParams memory settlementParams =
        ...
        emit IERC20.Transfer(address(handler), user, 1 ether);
        handler.handleV3AcrossMessage(baseToken, 1 ether, address(0), abi.encode(settlementParams));
    ...
    function test_swapAndCreatePosition_insufficientBalance() public {
        ...
        vm.expectRevert(ILPMigrationHandler.InsufficientBalance.selector);
        handler.swapAndCreatePosition(100, settlementParams);
    }
    ...
    function test_swapAndCreatePosition_bothAmountsZero() public {
        ...
        ILPMigrator.SettlementParams memory settlementParams =
            this.generateSettlementParams(0, 0, -200000, Range.InRange, true);
        vm.expectRevert(ILPMigrationHandler.AtLeastOneAmountMustBeGreaterThanZero.selector);
        handler.swapAndCreatePosition(1 ether, settlementParams);
    }

    function test_swapAndCreatePosition_token0BaseTokenInRange() public {
        // deal baseToken to handler
        deal(baseToken, address(handler), 1 ether);
        ...
        emit IERC20.Approval(address(handler), swapRouter, 0.5 ether);

        // Transfer, Transfer, Swap events from swap
        vm.expectEmit(true, true, false, false);
        emit IERC20.Transfer(address(pool), address(handler), 0);
        vm.expectEmit(true, true, false, false);
        emit IERC20.Transfer(address(handler), address(pool), 0);
        vm.expectEmit(true, true, false, false);
        emit IUniswapV3PoolEvents.Swap(address(swapRouter), address(handler), 0, 1, 0, 0, 0);

        // Approve for usdc and basetoken to nftPositionManager
        vm.expectEmit(true, true, false, false);
        emit IERC20.Approval(address(handler), nftPositionManager, 0);
        vm.expectEmit(true, true, false, false);
        emit IERC20.Approval(address(handler), nftPositionManager, 0);
        ...
        vm.expectEmit(true, true, false, false, address(baseToken));
        emit IERC20.Transfer(address(handler), address(user), 0);
  ...
  // deal baseToken to handler
        ...
        // Approve for basetoken to swaprouter
        vm.expectEmit(true, true, false, false, baseToken);
        emit IERC20.Approval(address(handler), swapRouter, 1 ether);

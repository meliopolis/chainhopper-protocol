...
import {AerodromeAcrossSettlerHarness} from "./mocks/AerodromeAcrossSettlerHarness.sol";
...
contract AerodromeAcrossSettlerTest is TestContext, AerodromeHelpers {
    ...
    AerodromeAcrossSettlerHarness public settler;
    uint256 public tokenId = 10574288;
    ...
    function setUp() public {
        ...
        vm.prank(owner);
        settler = new AerodromeAcrossSettlerHarness(
            owner,
            address(aerodromePositionManager),
    ...
    function genSettlerData(
        ...
        int24 currentTick =
            AerodromeHelpers.getCurrentTick(address(aerodromePositionManager), token0, token1, _tickSpacing);
        (migrationId, data) = SettlementHelpers.generateSettlerData(
            SettlementHelpers.generateAerodromeSettlementParamsUsingCurrentTick(
                user, token0, token1, _tickSpacing, 0, currentTick, range, 0, 0, isToken0BaseToken, senderShareBps
            ),
    ...
    function test_handleV3AcrossMessage_failsHashAndIsIgnored() public {
        ...
        (bytes32 migrationId, bytes memory data) =
            genSettlerData(weth, usdc, tickSpacing, SettlementHelpers.Range.InRange, true);
        ...
        vm.expectRevert(IAcrossSettler.InvalidMigration.selector);
    ...
    function test_handleV3AcrossMessage_ST_Token0BaseToken_failsAndRefunds() public {
        ...
        deal(weth, address(settler), wethAmount);
        (bytes32 migrationId, bytes memory data) =
            genSettlerDataWithAmounts(weth, usdc, tickSpacing, SettlementHelpers.Range.InRange, true, 1 ether, 1 ether);
    ...
    function test_handleV3AcrossMessage_ST_Token0BaseToken_InRange() public {
        ...
        emit Swap(address(aerodromeRouter), address(settler), 0, 1, 0, 0, 0);

        // Minting
        vm.expectEmit(true, false, false, false);
        emit Mint(address(settler), address(aerodromePositionManager), 0, 0, 0, 0, 0);
    ...
    function test_handleV3AcrossMessage_ST_Token0BaseToken_BelowCurrentTick() public {
        ...
        (bytes32 migrationId, bytes memory data) =
            genSettlerData(weth, usdc, tickSpacing, SettlementHelpers.Range.BelowTick, true);
    ...
    function test_handleV3AcrossMessage_ST_Token0BaseToken_AboveCurrentTick() public {
        ...
        (bytes32 migrationId, bytes memory data) =
            genSettlerData(weth, usdc, tickSpacing, SettlementHelpers.Range.AboveTick, true);
    ...
    function test_handleV3AcrossMessage_ST_Token0BaseToken_SingleSided_NewPool() public {
        ...
        vm.recordLogs();
        // generate data; above tick means only token0 is needed
        (bytes32 migrationId, bytes memory data) = genSettlerDataWithSqrtPriceX96(
            weth, newTokenSecond, tickSpacing, SettlementHelpers.Range.AboveTick, true, 2 ** 96
        );
        ...
        // pool creation event
        vm.expectEmit(true, true, false, false);
        emit PoolCreated(weth, newTokenSecond, tickSpacing, address(0));
    ...
    function test_handleV3AcrossMessage_ST_Token1BaseToken_failsAndRefunds() public {
        ...
        vm.recordLogs();
        // generate data with high amount0Min and amount1Min so it'll fail to mint
        (bytes32 migrationId, bytes memory data) = genSettlerDataWithAmounts(
            virtualToken, weth, tickSpacing, SettlementHelpers.Range.InRange, false, 1 ether, 1 ether
        );
    ...
    function test_handleV3AcrossMessage_ST_Token1BaseToken_InRange() public {
        ...
        // generate data
        (bytes32 migrationId, bytes memory data) =
            genSettlerData(virtualToken, weth, tickSpacing, SettlementHelpers.Range.InRange, false);
    ...
    function test_handleV3AcrossMessage_ST_Token1BaseToken_BelowCurrentTick() public {
        ...
        // generate data
        (bytes32 migrationId, bytes memory data) =
            genSettlerData(virtualToken, weth, tickSpacing, SettlementHelpers.Range.BelowTick, false);
    ...
    function test_handleV3AcrossMessage_ST_Token1BaseToken_AboveCurrentTick() public {
        ...
        // generate data
        (bytes32 migrationId, bytes memory data) =
            genSettlerData(virtualToken, weth, tickSpacing, SettlementHelpers.Range.AboveTick, false);
    ...
    function test_handleV3AcrossMessage_ST_Token1BaseToken_SingleSided_NewPool() public {
        ...
        // generate data; above tick means only token0 is needed
        (bytes32 migrationId, bytes memory data) = genSettlerDataWithSqrtPriceX96(
            newTokenFirst, weth, tickSpacing, SettlementHelpers.Range.BelowTick, false, 2 ** 96
        );
        ...
        // pool creation event
        vm.expectEmit(true, true, false, false);
        emit PoolCreated(newTokenFirst, weth, tickSpacing, address(0));
    ...
    function test_handleV3AcrossMessage_ST_NonWethBaseToken_failsAndRefunds() public {
        ...
        (bytes32 migrationId, bytes memory data) =
            genSettlerDataWithAmounts(usdc, usdt, tickSpacing, SettlementHelpers.Range.InRange, true, 1 ether, 1 ether);
    ...
    function test_handleV3AcrossMessage_ST_NonWethBaseToken_InRange() public {
        ...
        // generate data
        (bytes32 migrationId, bytes memory data) =
            genSettlerData(usdc, usdt, tickSpacing, SettlementHelpers.Range.InRange, true);
        ...
        // Transfer Position from 0x0 to user
        vm.expectEmit(true, true, true, false);
        emit IERC721.Transfer(address(0), user, tokenId + 1); // tokenId is incremented by 1 while creating the pool
        ...
        // Settlement
        vm.expectEmit(true, true, false, true);
        emit ISettler.Settlement(migrationId, user, tokenId + 1);
    ...
    function test_handleV3AcrossMessage_ST_NonWethBaseToken_BelowCurrentTick() public {
        ...
        // generate data
        (bytes32 migrationId, bytes memory data) =
            genSettlerData(usdc, usdt, tickSpacing, SettlementHelpers.Range.BelowTick, true);
    ...
    function test_handleV3AcrossMessage_ST_NonWethBaseToken_AboveCurrentTick() public {
        ...
        // generate data
        (bytes32 migrationId, bytes memory data) =
            genSettlerData(usdc, usdt, tickSpacing, SettlementHelpers.Range.AboveTick, true);
    ...
    function test_handleV3AcrossMessage_ST_NonWethBaseToken_SingleSided_NewPool() public {
        ...
        // generate data; above tick means only token0 is needed
        (bytes32 migrationId, bytes memory data) = genSettlerDataWithSqrtPriceX96(
            usdc, newTokenSecond, tickSpacing, SettlementHelpers.Range.AboveTick, true, 2 ** 96
        );
        ...
        // pool creation event
        vm.expectEmit(true, true, false, false);
        emit PoolCreated(usdc, newTokenSecond, tickSpacing, address(0));
    ...
    function test_handleV3AcrossMessage_DT_FirstBridgeCallFailsWhenFirstAmountTooLowAndIsIgnored() public {
        ...
        (bytes32 migrationId, bytes memory data) = genSettlerDataForDualToken(
            weth, usdc, tickSpacing, SettlementHelpers.Range.InRange, true, wethAmount - 1, usdcAmount - 1
        );
    ...
    function test_handleV3AcrossMessage_DT_SecondBridgeCallFailsSecondAmountTooLowAndIsIgnored() public {
        ...
        (bytes32 migrationId, bytes memory data) = genSettlerDataForDualTokenWithAmounts(
            ...
            usdc,
            tickSpacing,
            SettlementHelpers.Range.InRange,
            true,
            wethAmount,
            usdcAmount,
            wethAmount,
    ...
    function test_handleV3AcrossMessage_DT_InRange_ExistingPool_Token0ArrivesBeforeToken1() public {
        ...
        // generate data
        (bytes32 migrationId, bytes memory data) = genSettlerDataForDualToken(
            weth, usdc, tickSpacing, SettlementHelpers.Range.InRange, true, wethAmount, usdcAmount
        );
    ...
    function test_handleV3AcrossMessage_DT_InRange_NewPool_Token0ArrivesBeforeToken1() public {
        ...
        (bytes32 migrationId, bytes memory data) = genSettlerDataForDualTokenWithSqrtPriceX96(
            weth, address(mockUSDC), tickSpacing, SettlementHelpers.Range.InRange, true, wethAmount, usdcAmount, 2 ** 96
        );
        ...
        // pool creation event
        vm.expectEmit(true, true, false, false);
        emit PoolCreated(weth, address(mockUSDC), tickSpacing, address(0));
    ...
    function test() public override(TestContext, AerodromeHelpers) {}
}

...
contract AerodromeAcrossMigratorTest is TestContext, AerodromeHelpers {
    event Swap(
    ...
    AerodromeAcrossMigrator public migrator;
    ...
    function setUp() public {
        ...
        vm.prank(owner);
        migrator = new AerodromeAcrossMigrator(
            owner,
    ...
    */
    ...
    function test_onERC721Received_Token0WETHBaseToken_InRange() public {
        ...
        (uint256 tokenId, uint256 amount0,) = mintAerodromePosition(
            address(aerodromePositionManager), user, token0, token1, -250000, -100000, tickSpacing, 0
        );

        // verify posToken0 is baseToken
        (,, address posToken0,,,,,,,,,) = aerodromePositionManager.positions(tokenId);
        ...
        vm.expectEmit(true, true, false, false, address(aerodromePositionManager));
        emit IERC721.Transfer(user, address(migrator), tokenId);

        // Burn
        vm.expectEmit(true, false, false, false, address(aerodromePositionManager));
        emit IAerodromePositionManager.DecreaseLiquidity(tokenId, 0, 0, 0);

        // collect
        vm.expectEmit(true, false, false, false, address(aerodromePositionManager));
        emit IAerodromePositionManager.Collect(tokenId, address(0), 0, 0);

        // Swap
        vm.expectEmit(true, true, false, false);
        emit Swap(address(aerodromeRouter), address(migrator), 0, 0, 0, 0, 0);
        ...
        vm.prank(user);
        aerodromePositionManager.safeTransferFrom(user, address(migrator), tokenId, abi.encode(migrationParams));
    ...
    function test_onERC721Received_Token0WETHBaseToken_BelowCurrentTick() public {
        ...
        (uint256 tokenId, uint256 amount0,) = mintAerodromePosition(
            address(aerodromePositionManager), user, token0, token1, -250000, -210000, tickSpacing, 0
        );
        // only token1 is used
    ...
    function test_onERC721Received_Token0WETHBaseToken_AboveCurrentTick() public {
        ...
        // current tick is ~ -201000
        (uint256 tokenId, uint256 amount0,) = mintAerodromePosition(
            address(aerodromePositionManager), user, token0, token1, -150000, -100000, tickSpacing, 0
        );
    ...
    function test_onERC721Received_Token1WETHBaseToken_InRange() public {
        ...
        address token1 = weth;
        (uint256 tokenId,, uint256 amount1) = mintAerodromePosition(
            address(aerodromePositionManager), user, token0, token1, -200000, -5000, tickSpacing, 0
        );

        // verify posToken0 is baseToken
        (,,, address posToken1,,,,,,,,) = aerodromePositionManager.positions(tokenId);
    ...
    function test_onERC721Received_Token1WETHBaseToken_BelowCurrentTick() public {
        ...
        address token1 = weth;
        (uint256 tokenId,, uint256 amount1) = mintAerodromePosition(
            address(aerodromePositionManager), user, token0, token1, -200000, -100000, tickSpacing, 0
        );
    ...
    function test_onERC721Received_Token0USDCBaseToken_NoWETH_InRange() public {
        ...
        address token1 = usdt;
        // current tick is ~ -201000
        // extra position to privide liquidity
        mintAerodromePosition(
            address(aerodromePositionManager), user, token0, token1, -5000, 5000, tickSpacing, sqrtPriceX96
        );
        (uint256 tokenId, uint256 amount0,) =
            mintAerodromePosition(address(aerodromePositionManager), user, token0, token1, -5000, 5000, tickSpacing, 0);
    ...
    function test_onERC721Received_Token0USDCBaseToken_NoWETH_BelowCurrentTick() public {
        ...
        (uint256 tokenId, uint256 amount0,) =
            mintAerodromePosition(address(aerodromePositionManager), user, token0, token1, -1000, -100, tickSpacing, 0);
    ...
    function test_onERC721Received_Token0USDCBaseToken_NoWETH_AboveCurrentTick() public {
        ...
        (uint256 tokenId, uint256 amount0,) =
            mintAerodromePosition(address(aerodromePositionManager), user, token0, token1, 100, 500, tickSpacing, 0);
    ...
    function test_onERC721Received_Token0WETHBaseToken_Token1USDCBaseToken_InRange() public {
        ...
        address token1 = usdc;
        // current tick is ~ -201000
        (uint256 tokenId, uint256 amount0, uint256 amount1) = mintAerodromePosition(
            address(aerodromePositionManager), user, token0, token1, -250000, -100000, tickSpacing, 0
        );

        // verify posToken0 is baseToken
        (,, address posToken0, address posToken1,,,,,,,,) = aerodromePositionManager.positions(tokenId);
        assertEq(posToken0, token0);
        assertEq(posToken1, token1);
